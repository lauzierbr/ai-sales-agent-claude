...
| F-05 | **AnalystAgent — meta-agente de observabilidade** — persona admin que consulta Langfuse e extrai inteligência operacional (custo, anomalias, qualidade). Diferencial de produto. Aprovado em ADR D031 + D032 (interface web + NL2SQL), alvo Sprint 11. NL2SQL resolve B-41/B-42/F-08 transitivamente. | Investigação 29/04 | Alto — diferencial competitivo + reduz tempo de investigação + reduz custo marginal de novas perguntas analíticas a zero | Alto (3 sprints incrementais) |
...
| F-08 | **Clientes sem pedidos há N dias** — tool `clientes_sem_pedido_efos(dias, top_n)` para o AgentGestor: identifica clientes que compraram historicamente mas não têm pedido em `commerce_orders` nos últimos N dias. Distinto de "clientes inativos" (situação=2 no EFOS = status fiscal/comercial); este é **inatividade comportamental** — churn por ausência de compra recente. Query: LEFT JOIN `commerce_accounts_b2b` com `commerce_orders` filtrando `data_pedido >= NOW() - INTERVAL N days`; clientes sem match = parados. Parâmetro `N` configurável (padrão 90 dias). Pode complementar com última data de compra (`dataultimacompra` já em `commerce_accounts_b2b` após Sprint 10). | Piloto 01/05 | Alto — instrumento de retenção/reativação direto pelo gestor via WhatsApp | Baixo |
