...
| pytest staging | `pytest -m staging` (macmini-lablz) | **FAIL** — 12 falhas / 17 pass / 3 skip (ver categorização abaixo) |
| M_INJECT | `pytest test_ui_injection.py` (macmini-lablz) | **FAIL** — arquivo não existe no macmini-lablz |
| smoke gate A_SMOKE | `smoke_sprint_6.py` (macmini-lablz) | **NÃO EXECUTADO** — código não deployado |
| smoke gate A10 | `smoke_ui.sh` (macmini-lablz) | **NÃO EXECUTADO** — código não deployado |
...
**Motivo:** `scripts/smoke_ui.sh` existe no repositório ✅, mas o app não está rodando no macmini-lablz com código do Sprint 6. Mac-lablz está no commit `5355ee8` (Sprint 5-hotfix). Sprint 6 não foi sincronizado.
...
**Arquivo no macmini-lablz:** NÃO EXISTE (código não sincronizado)
**Execução staging:** IMPOSSÍVEL — `pytest test_ui_injection.py` retornou "file or directory not found" no macmini-lablz
...
O `pytest -m staging` rodou no macmini-lablz com código PRE-Sprint 6 (commit `5355ee8`). As 12 falhas são:
...
| Staging executado | CANNOT RUN | FAIL — código não no macmini-lablz |
...
2. **Testes staging e smoke gate não podem ser executados** enquanto o código não estiver no macmini-lablz.
...
```bash
# No macmini-lablz, sincronizar código do Sprint 6:
...
O Generator deve: (1) sincronizar código para macmini-lablz via `git pull` + `deploy.sh staging`, (2) rodar `seed_homologacao_sprint-6.py`, (3) confirmar que `smoke_sprint_6.py` retorna ALL OK antes de repassar ao Evaluator.
...
```bash
# Após `git pull` no macmini-lablz:
